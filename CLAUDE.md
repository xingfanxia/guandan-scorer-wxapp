...
**当前状态（2026-06-27）：微信认证已通过（名称 + 资质双成功）→「被分享」解锁，分享卡片自动进房真机验证通过；核心闭环 + 档案对齐 web 三段 + 围观读路径加固（room_get）+ 续6 战绩长图真机导出修复**（分支 wxapp-2-scoring-loop 已推 origin；**续5/1.0.9 + 续6/1.0.10 已 merge 到 main**，详细见下方 2026-06-21/续5、2026-06-27/续6 段）——单机计分闭环、云房间围观（watch+轮询）、投票/座位认领/档案、长图海报（web 手机版对位）、玩家池与 web 数据迁移、天梯分（简化 ELO + 起评分；**校准门按历史总场次 web+小程序合计 <3 才待校准沉底**；单一 vendor 源 `shared/ladderLogic.js`）、玩家天梯查询页、**外观开关**（首页三段控件覆盖系统主题，机制见 DESIGN.md §1）。
...
**2026-06-19（续4）分享卡片自动进房 = 认证门，非代码（官方确认）**：第二个号点「...」转发卡片落首页空房（同房号「围观别人」手输能进完整局 → 纯路由、云端有数据非 push bug）。根因 = **未认证小程序无「被分享」能力**（官方「快速创建个人小程序」文档 + mp 后台微信认证页明写：完成微信认证后才获「被搜索/被分享」），微信把 `onShareAppMessage` 自定义 title+path 丢了走默认（当前页 index）→ 落首页。**代码 path 一直对的，绕不过；认证是唯一正解**（个人 30 元/年扫脸，~1 小时电话确认）。**不改纯数字房号**（只优化手输、要重部署 8 云函数、仍不解卡片）。客户端 **体验版 1.0.8**。详见 docs/PLAN.md 续4。

**2026-06-21（续5）微信认证已通过 →「被分享」解锁**：收到「小程序微信认证-名称认证成功通知」+「资质认证成功通知」两条。认证前微信丢弃 `onShareAppMessage` 的自定义 title+path 走默认（落首页），现解锁后应保留。**已复核分享代码端到端正确、无需改代码**：产出端 `index.ts:120` / `room.ts:217`（`path:/pages/room/room?code=${code}`），落点 `room.ts:44 onLoad` 读 `options.code` → `watchRoom` → `room_get` 保底读（非房主也能读）。**待 AX：mp 后台「版本管理」设 1.0.8 为体验版 → 用第一个号「...」转发一个有内容的房间 → 第二个号点卡片，应直接落围观页（带房号实时比分），不再落首页空房。** 若仍落首页 → 先确认卡片标题是自定义「闹掼计分 · 房间 XXX…」而非默认 app 名（默认名=认证态/path 仍未生效）。**真机验证：第二个号点卡片已能直接进房（用户确认）。** 随之**下线首页「🔭 围观房间码」手输入口**（`onEnterRoomCode` handler + `.share__watch` 样式一并删除）——它本是认证前分享卡片走不通的兜底，认证后分享卡片成为唯一围观入口；牌友漏接卡片时房主重新「发到群里」即可。**同轮再精简两处 UI + 一处文案**：① 下线首页「开围观」手动建房按钮（`onOpenRoom` 删除）—— 房间改由开打记首局自动建（`index.ts:640` 既有逻辑），围观分享区仅留「发到群里」，无房时显示「开打后自动生成房间码」提示（代价：开打前不能预建房，自动建失败下一局重试自愈）；② 荣誉 caption 去英文尾注（`honorDisplay.js` 16 条 `· English` 删除，纯中文）。**→ 切体验版 1.0.9**（= 认证后分享卡片生效 + 下线手输围观 + 下线开围观 + 荣誉去英文）；311 测试 + tsc 绿；commit `32b6a73` 已推 origin、**1.0.9 已 `cli upload` 成功（175.8KB）**。**待 AX：mp 后台「版本管理」设 1.0.9 为体验版。**

**2026-06-27（续6）战绩长图真机导不出修复 + 全量 merge 到 main**：用户报 1.0.9 真机「生成战绩海报存相册」失败、无法生成。**根因 = 海报离屏 `type="2d"` canvas 背板尺寸超设备 GPU `maxTextureSize`**：背板高 = `layout.height × scale`，逐局历史越多越高；旧守卫只在 `height×dpr>8000` 时把 scale 降到 1，且从不约束背板原始高本身。真机 `canvasToTempFilePath` 在背板任一边超上限（低/中端机常见 **4096**，非旧代码假设的 8192）即导不出 —— dpr=2 下哪怕 4 局短海报（~2360 布局 px → 4720 背板 px）也已超 4096，故真机基本必败；此前只在模拟器（桌面 GPU 上限高）测过 → 上线即坏（非 1.0.9 引入，是历来都坏）。**修复**：`core/poster.js` 抽纯函数 `computePosterScale(w,h,dpr,maxDim=4096)` 按最长边把背板压进上限（超长历史允许 `scale<1`，清晰度降级好过导不出，`MAX_CANVAS_DIM=4096` 兜全机型）；`index.ts onSavePoster` 改用它 + `canvas.width/height = round(layout×scale)` + `destWidth/destHeight = canvas.width/height`；`canvasToTempFilePath` fail 分支改 loud（`console.error` 打 errMsg + 背板尺寸留证）。新增 5 条不变式测试 → **316 测试 + tsc 绿，code-reviewer APPROVE**。commit `7fece1d`。**本轮按惯例把续5/1.0.9 + 续6/1.0.10 全量 merge 到 main**（main 此前停在续4/1.0.8，sync commit `4bbbb46`）。**1.0.10 已 `cli upload` 成功（176.2KB）**。**待 AX：mp 后台「版本管理」设 1.0.10 为体验版 → 真机打完一局点「生成战绩海报存相册」验证能存相册。**
...
- **云开发环境**：`cloud1-d2go4yxtv833a2113`（微信云开发免费环境；WXAPP-3 云函数/数据库用）
- **免费计费规则（官方「计费说明」核实 2026-06-19，来源 developers.weixin.qq.com/.../wxcloud/billing/price.html）**：免费环境在小程序**未发布上线阶段（开发版/体验版）= 无限期免费、不续费**（活动截止 2026-12-31）；**正式版发布上线后，免费环境到期时间 = 上线后第 15 天**，过期要继续用须转付费。转付费底价 = **基础套餐 19.9 元/月**（原价 39，5 折长期有效），配额 20 万次调用 + 2GB 容量 + 2GB 云函数出流量 + 10 万 GBs + 5GB CDN + 500 QPS；超出才叠加按量（0.5 元/万次等）。**朋友局计分器量级（月几千次读写）远低于 20 万次 → 实际就是 19.9/月一口价、不触发按量。** **触发收费的是「发布正式版上线」动作，与微信认证、ICP 备案无关**（认证=解锁被分享，备案=合法域名，都不直接产生云开发费用）。停在体验版给朋友用 = 永久免费。
