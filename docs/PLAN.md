...
- 2026-06-14（续）：**全部部署 + 绑定档案合并修复 + 体验版 1.0.5**（commits `5cba356` 修复 + `83a5d39`，main 对齐 `c8ba911`）。
  - **部署阻塞复盘**：先 `ResourceNotFound.Namespace`（云会话漂到别 env）→ 重启 IDE 后转 `getCloudAPISignedHeader 41002 system error`（微信签名后端抽风，持续数小时）。根因之一：**开了两个 DevTools 实例**（CLI 命中坏会话那个）—— 杀掉全部 DevTools（保留 WeChat 客户端）单实例重开后，等微信签名后端自愈，6 个云函数一把部署成功。**教训**：41002 持续时先查是否多实例（`lsof -i | grep wechatweb`），再等后端恢复，别空转重试。
...
- 2026-06-19：**分享后非房主进不去房间 —— 根因 + 加固**（用户报告「巨大问题：share 到群里别人打开/手输房间码都进不去」）。
  ...
  - code-reviewer 通过（无 CRITICAL/HIGH）；M1（认领后 watch 帧覆盖 mine 闪烁）→ `watchRoom.syncVersion` 同步去重游标修复；L1（version 自增不变量）补注；L2（db_error errMsg 不下发客户端）。311 测试（+6 roomView）+ tsc 绿。**待 AX：部署 `room_get` + 上传新客户端 + 控制台设 rooms 读权限。**
