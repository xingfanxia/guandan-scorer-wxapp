...
`shared-logic/` ← 复制 web repo 顶层 `shared/`（994 行，零宿主依赖）+ `src/game/calculator.js`（231 行纯函数）+ 从 `rules.js`/`honors.js`/`statistics.js` 抽出的纯算法。每次同步在文件头注记 upstream commit hash。
...
→ verify: 同一局数据在 web 版与小程序版算出相同荣誉得主。
