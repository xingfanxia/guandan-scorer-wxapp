...
读路径（围观）不走云函数：客户端直接 `db.watch` rooms 文档（读权限"所有用户可读，仅创建者可写"，人工清单 #2），省调用量。
...
### WXAPP-9：玩家天梯分（简化 ELO，spec 2026-06-12 定稿）
1. 每人初始 1000 分，按「场」（整场通关）结算，不按局。
2. 队伍分 = 全队成员当前天梯分均值（未绑定/新玩家按 1000 计入均值）。
3. 期望胜率 E = 1/(1+10^((对队均分−己队均分)/400))；胜 S=1，负 S=0。
4. 队伍增量 = 32×(S−E) —— 强队赢弱队 E 大、加分少；爆冷多得。两队增量互为相反数。
5. 个人表现增量 = 16×(中位名次−场均名次)/(人数−1)，区间约 ±8 —— 同队内打得好的多拿/少扣。
6. 个人增量 = round(队伍增量+个人表现增量)；存 players.stats.ladder = {rating, sessions, peak}。
7. 幂等跟 gameKey 同一道闸（applySession 去重时一并应用/跳过）；rating 下限 0。
8. 纯函数 `core/ladder.js`（Node 可测）；服务端 profile_sync CJS 镜像同算法（同 voteSessionKey 镜像先例），评分读写全在服务端。
→ verify: 纯函数测试覆盖 强胜弱少加分/弱胜强多加分/零和/个人表现拉开同队差距/幂等；档案页 + 查询 view 展示天梯分。

### 下一 session 工作清单（2026-06-12 暂停点，用户 6 条 feedback 盘点后的余项）

1. **房间与会话彻底分离**：开新一局（重置/换人数）→ 旧房间封存（finishedAt），自动开新房间；所有 round/统计/荣誉/投票严格跟房间走。（当前：reset 后仍推送同一房间，会覆盖旧局快照 —— 语义不对）
2. **玩家战绩查询 view**：看任意池内玩家的档案（web players.html 对位）—— pool_list 已有入口数据，缺 profile_get_by_handle 函数 + 查询页。
3. **海报信息密度对齐 web 长图**：加每玩家统计表 + 完整逐局列表（保持简洁大方调性，纵向预算机制已就绪）。
4. **WXAPP-9 天梯分**（见上）。
5. 复盘 review 余项：profileSession claims 映射测试、海报合规测试已加；profile_sync 服务端白名单已实装 —— 下一轮 review 盯云函数新增面。
...
1. **部署 9 个云函数**：DevTools 资源管理器右键 cloudfunctions/ 下各目录 → 上传并部署（云端安装依赖）：`vote_submit / vote_tally / vote_reset / room_claim_seat / profile_sync / profile_get / pool_import / pool_list / pool_bind`（room_create/room_write 已部署但有更新，也重传一次）。CLI 部署 2026-06-12 因 mp 云会话故障（41002/Namespace）持续失败，执行面正常 —— 换个时间 `cli cloud functions deploy --env cloud1-d2go4yxtv833a2113 --names room_create room_write vote_submit vote_tally vote_reset room_claim_seat profile_sync profile_get pool_import pool_list pool_bind --remote-npm-install --project $(pwd)` 多半就过。
