...
- 2026-06-19（续3）：**真机迭代 —— 3 个生命周期 bug + 围观体验优化（体验版 1.0.7）**（双号实测反馈；commits `4196c49` + `c1c7b54`）。
  ...
  - **本轮无云改动**（纯客户端，room_get 等已 live）。311 测试 + tsc 绿；code-reviewer 通过（无 CRITICAL/HIGH，仅 1 条 redundant-setData LOW 留作 resilience + 1 条 onShow 注释已补）。**待 AX**：① mp 后台选 **1.0.7** 为体验版 ② 测「3dot 转发卡片能否自动进房」——能则房号保持，不能则改 6 位纯数字（带 dedup）。UI/生命周期改动无单测（无 wx mock harness），靠 tsc + 真机复测。
