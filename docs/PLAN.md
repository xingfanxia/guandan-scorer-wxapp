...
### WXAPP-9：玩家天梯分（用户拍板新增，未开始 —— 下一 session）
简化 ELO：每场结束按综合战绩加减天梯分。参考因素：自己战绩 + 其他玩家表现 + 己队均分 vs 对队均分（强队赢弱队加分少）。纯函数实现进 core/（Node 可测），分数存 players.stats.ladder，档案页 + 玩家查询 view 展示。设计要点下一 session 先出小 spec 再写。
