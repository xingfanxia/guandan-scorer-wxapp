...
- 2026-06-19：**分享后非房主进不去房间 —— 根因 + 加固**（用户报告「巨大问题：share 到群里别人打开/手输房间码都进不去」）。
  ...
  - **客户端已上传体验版 1.0.6**（`cli upload --version 1.0.6`，room_get 集成生效）。注意：`cli upload` 客户端通道正常，**只有 `cli cloud functions deploy` 的签名通道 41002**——再次坐实根因是云函数部署专属。控制台 rooms 读权限已设（用户 2026-06-19 当场改为「所有用户可读」）。**待 AX**：mp 后台 → 版本管理 → 选 1.0.6 为体验版（让体验成员拿到新包）。分享当前已可用（权限+watch），room_get 是加固。
