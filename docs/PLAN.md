...
8. **起评分**（无小程序战绩时从 web 历史折算，同口径名次为主）：1000 + 置信度×(250×(4.5−场均名次)/3.5 + 300×(胜率−0.5))，钳 [700,1300]，置信度 = min(场次,20)/20；只在 sessions=0 时垫底，永不覆盖已挣分。
9. 纯函数 `core/ladder.js`（Node 可测）；服务端 profile_sync / pool_bind / pool_list CJS 镜像同算法（同 voteSessionKey 镜像先例），评分读写全在服务端。
