...
- 2026-06-19：**分享后非房主进不去房间 —— 根因 + 加固**（用户报告「巨大问题：share 到群里别人打开/手输房间码都进不去」）。
  ...
  - **客户端已上传体验版 1.0.6**（`cli upload --version 1.0.6`，room_get 集成生效）。注意：`cli upload` 客户端通道正常，**只有 `cli cloud functions deploy` 的签名通道 41002**——再次坐实根因是云函数部署专属。控制台 rooms 读权限已设（用户 2026-06-19 当场改为「所有用户可读」）。分享当前已可用（权限+watch），room_get 是加固。
- 2026-06-19（续3）：**真机迭代 —— 3 个生命周期 bug + 围观体验优化（体验版 1.0.7）**（双号实测反馈；commits `4196c49` + `c1c7b54`）。
  - **认证答疑**：「发到群里」报「未完成认证，分享功能暂时无法使用」= **微信平台限制**（未认证小程序 `open-type="share"` 被拦，人工清单 #3 已记），非本项目代码。绕过 = 「...」菜单转发 / 「围观别人」手输房号；解锁 = 小程序认证 30 元/年。
  - **bug 1**：「清空玩家重来」只 `resetGame(false)` 不 `detach()` → 留旧房间（这是云端 D8P4W5 空 8 人快照来源）。修：`clear` 分支 `detach()` 开新房；`keep`（重新开一局）保留房间。
  - **bug 2**：「围观别人」与「进房间」`wx:if/else` 互斥 → 有房间就点不了围观别人。修：「围观别人」做成围观卡里**显眼常驻主按钮**；nav 仅留「进我的房间」。
  - **bug 3（防分叉根因）**：`page.data.roomCode` 与会话 code 分叉（后台 room_not_found 清 code 不通知页面 → 分享/进房间引到失效房间）。修：`ownerRoom.subscribeOwnerCode` 订阅，`index.onLoad` 注册（onUnload 清理）→ 房号锁步。`onShow` 仍直读一次 seed 初始值（storage 恢复不发订阅，注释勿删）。
  - **围观页优化**（image 12）：座位认领区 → **红蓝队阵容**（按队分组）；荣誉满 5 局解锁，未满显示「还差 X 局」提示；**认领 client 下线**（`room_claim_seat` 云函数保留休眠，SeatVM 瘦身为 id/name/emoji/team 仅供投票）。
  - **本轮无云改动**（纯客户端，room_get 等已 live）。311 测试 + tsc 绿；code-reviewer 通过（无 CRITICAL/HIGH，仅 1 条 redundant-setData LOW 留作 resilience + 1 条 onShow 注释已补）。UI/生命周期改动无单测（无 wx mock harness），靠 tsc + 真机复测。
- 2026-06-19（续4）：**分享卡片点开落首页空房 = 未认证，非代码 bug（官方确认）→ 用户已提交微信认证（审核中）**。
  - **症状**：第二个号点「...」转发的卡片 → 落首页空房；同一房号「围观别人」手输能进完整局 → **纯路由问题，云端房间有数据（非 push bug）**。
  - **根因（官方铁证）**：卡片标题是默认 app 名「闹掼计分器」而非 `onShareAppMessage` 设的自定义标题 → 微信把自定义 title+path 整个丢了，走默认转发（当前页 index 路径）。官方文档「快速创建个人小程序」明写：**自 2023-12-25 起新注册账号默认未认证，需完成认证后方可获得「被搜索、被分享」能力**；mp 后台微信认证页也写「完成微信认证后账号可获得『被搜索』『被分享』能力」。∴ **未认证 = 无自定义分享，认证后解锁，代码无法绕过**（`path` 在代码里一直是对的）。「围观别人」手输能进是因为走 app 内 `navigateTo`，不经分享层。
  - **决定**：**不改纯数字房号**（那只优化手输、需改 8 个云函数 regex + room_create + GUI 重部署，且仍不解卡片自动进房）。认证（个人/个体 30 元/年，法人扫脸，~1 小时一个确认电话）是「点卡片即进」的唯一正解。
  - **状态（续4 当时）**：微信认证审核中（用户已提交+扫脸+付费）；客户端**体验版 1.0.8**（补单行「围观房间码」按钮）。→ 见续5，认证已于 2026-06-21 通过。
- 2026-06-21（续5）：**微信认证通过 →「被分享」解锁，分享卡片自动进房应生效**。
  - 收到两条通知：「小程序微信认证-名称认证成功通知」+「小程序微信认证-资质认证成功通知」。续4 根因（未认证无「被分享」、微信丢自定义 title+path）随认证消除。
  - **分享代码端到端复核正确、无需改代码**：产出端 `index.ts:120`（首页有房）/ `room.ts:217`（围观页再转发）均 `path:/pages/room/room?code=${code}`；落点 `room.ts:44 onLoad` 读 `options.code`（trim+大写）→ `watchRoom(code)` → `room_get` 云函数保底读（非房主 openid 也能读，watch 仅实时优化）。
  - **待 AX（验收三步）**：① mp 后台「版本管理」设 1.0.8 为体验版 → ② 第一个号「...」转发一个有内容的房间 → ③ 第二个号点卡片，应直接落围观页（带房号实时比分），不再落首页空房。
  - **失败判据**：若仍落首页，先看卡片标题——自定义「闹掼计分 · 房间 XXX，来围观比分」=path 生效（查别的）；仍是默认 app 名「闹掼计分器」=认证态/体验版未刷新（确认 1.0.8 已设为体验版、重进小程序）。
  - **真机已验证**：用户确认第二个号点卡片直接落围观页。**随即下线首页「🔭 围观房间码」手输入口**——删 `index.wxml` 按钮 + `index.ts onEnterRoomCode` handler + `index.wxss .share__watch` 样式（顺手修 nav 栅格注释 围观别人→进我的房间）。理由：手输入口本是认证前卡片走不通的兜底，认证后分享卡片成唯一围观入口；牌友漏接卡片 → 房主「发到群里」重发即可。311 测试 + tsc 绿，无残留引用。
  - **同轮再精简**：① 下线首页「开围观」手动建房按钮（删 `index.wxml` wx:else 支 + `index.ts onOpenRoom`）—— 房间改由开打记首局自动建（`index.ts:640` 既有），围观分享区只剩「发到群里」，无房时空状态提示改「开打后自动生成房间码，再发到群里给牌友围观」。代价：开打前不能预建房；自动建失败下一局自愈。② 荣誉 caption 去英文尾注（`honorDisplay.js` 16 条 `· English` 删，纯中文；测试只断言非空+不含「赌」，不破）。
  - **→ 体验版 1.0.9 切版**：= 认证后分享卡片生效 + 下线手输围观 + 下线开围观 + 荣誉去英文。311 测试 + tsc 绿；commit `32b6a73` 已推 origin、**1.0.9 已 `cli upload` 成功（175.8KB）**。**待 AX：mp 后台设 1.0.9 为体验版。**
  - 备案与计费独立：认证 ≠ 触发云开发收费；停体验版仍永久免费（见 memory cloud-free-tier-ends-15d-after-launch）。
- 2026-06-27（续6）：**战绩长图真机导不出修复 + 全量 merge 到 main**。
  - **根因**：海报离屏 `type="2d"` canvas 背板 = `layout.height × scale`，逐局历史越多越高；旧守卫只在 `height×dpr>8000` 时降 scale 到 1，从不约束背板原始高。真机 `canvasToTempFilePath` 在背板任一边超设备 GPU `maxTextureSize`（低/中端机常见 **4096**，非旧代码假设 8192）即导不出 —— dpr=2 下哪怕 4 局短海报（~2360 布局 px → 4720 背板 px）也已超 4096 → 真机基本必败；只在模拟器（桌面 GPU 上限高）测过 → 历来都坏（非 1.0.9 引入）。
  - **修复**：`core/poster.js` 抽纯 `computePosterScale(w,h,dpr,maxDim=4096)`（最长边压进上限，超长历史允许 `scale<1`，`MAX_CANVAS_DIM=4096` 兜全机型）；`index.ts onSavePoster` 改用它 + `round(layout×scale)` 背板 + `destWidth/destHeight=canvas.{w,h}`；fail 分支 `console.error` 留证。新增 5 条不变式测试 → **316 测试 + tsc 绿，code-reviewer APPROVE**。commit `7fece1d`。
  - **merge**：按惯例（main = 续N sync 镜像，分支为超集）把续5/1.0.9 + 续6/1.0.10 全量 merge 到 main。
  - **1.0.10 已 `cli upload` 成功（176.2KB，sync commit `4bbbb46` 后上传）**。**待 AX**：mp 后台「版本管理」设 1.0.10 为体验版 → 真机打完一局点「生成战绩海报存相册」验证存相册成功（若仍失败，新 `console.error` 会打 errMsg + 背板尺寸，便于定位）。
