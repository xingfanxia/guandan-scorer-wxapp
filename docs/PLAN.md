...
- 2026-06-14：**档案对齐 web 三段 + 三个用户问题修复**（gap 审计 workflow → commits `da9b814` / `4f101a3` / `32cd84d`）。
  ...
  - 状态：299 测试 + tsc + sync:check 绿。**已部署 live**：profile_get / profile_get_by_handle（档案三段 cloud 侧）、pool_add(v2 基础)。**待部署（云会话又漂到别 env，ResourceNotFound.Namespace，同 task#24，等 dahua 释放会话）**：pool_add(genHandle test 前缀 + callerBound) / pool_prune(scanTest) / profile_sync(审核队列) / admin(新)。**待部署后**：批量部署这 4 个 + 清遗留 test 池玩家（`pool_prune {scanTest:true}`，有个 `test_勿留勿用` m-handle 遗留）+ 上传新客户端（含 admin 页/审核提示/绑定确认）+ profile-extras-verify E2E 真机验证 + main 对齐 + 新体验版。**注意**：客户端新功能（admin 页/审核队列/auto-bind 确认）在对应云函数部署前是 dormant/降级态，故先部署云函数再上传客户端，避免半成品。
