...
- 2026-06-19：**分享后非房主进不去房间 —— 根因 + 加固**（用户报告「巨大问题：share 到群里别人打开/手输房间码都进不去」）。
  ...
  - **待 AX**：① 上传新客户端（体验版）让 room_get 生效 ② 客户端上传前在 GUI「云端测试」room_get 传 `{"code":"A1B2C3"}` 应回 `{ok:false,error:"room_not_found"}`（顺带验证 `wx-server-sdk` 云端依赖已装）③ 控制台 rooms 读权限已设（用户 2026-06-19 当场改为「所有用户可读」）。分享当前已可用（权限+watch），room_get 是加固。
