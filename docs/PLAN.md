...
1. ~~**部署云函数**~~ ✅ 13 个云函数全部部署 live（room_create / room_write / vote_submit / vote_tally / vote_reset / room_claim_seat / profile_sync / profile_get / profile_get_by_handle / pool_import / pool_list / pool_bind / pool_prune）。CLI 部署命令：`cli cloud functions deploy --env cloud1-d2go4yxtv833a2113 --names <fn...> --remote-npm-install --project $(pwd)`。**坑（与 dahua-dice 共用 IDE 云会话）**：偶发 `ResourceNotFound.Namespace`（会话漂移到别的 env），执行面（callFunction）仍正常 —— 重试 / 等对方释放会话 / GUI 上传并部署。**标准动作**：`sync:shared` 改了 ladderLogic 后，必须重部署 4 个天梯云函数（profile_sync / pool_bind / pool_list / profile_get_by_handle，各打包 `ladderLogic.js`）。
