...
| 纯逻辑层 | 顶层 `shared/`（994 行：achievementLogic/gameStatus/honorCatalog/roomSnapshotValidation/ruleConfig/voteSessionKey）+ `src/game/calculator.js`（231 行纯函数） | **原样 vendor 进 `shared-logic/`**——零 DOM 依赖直接跑；upstream（web repo）是单一事实源，改规则先改那边再同步 |
| 规则引擎 | `src/game/rules.js`（算法与 state 单例/events 耦合） | 抽出 checkALevelRules/applyGameResult 纯算法部分，注入 state 快照 |
